...
def image_trim(im, boxes):
    x1 = boxes[:,0]
    y1 = boxes[:,1]
    x2 = boxes[:,2]
    y2 = boxes[:,3]
    for i in range(x1.shape[0]):
        cropImage = im[(int(y1[i])):(int(y2[i])), (int(x1[i])):(int(x2[i]))]
        blur = cv2.medianBlur(cropImage, 33)
        im[(int(y1[i])):(int(y2[i])), (int(x1[i])):(int(x2[i]))] = blur
    return im
...
def initFaceDetector():
    minsize = 20
    caffe_model_path = "/home/jieun/mtcnn-master/model"
...
def main():
    ...
    path_dir = '/home/jieun/caffe/python/ssdam/training-images/Unknowns'
    file_list = os.listdir(path_dir)
    file_list.sort()
    f = open("imglist_Unknowns.txt", 'w')
    for i in file_list:
        f.write(i + '\n')
    f.close()

    path_dir = '/home/jieun/caffe/python/ssdam/imageframes/sample_video_mp4'
    file_list = os.listdir(path_dir)
    file_list.sort()
    f = open("imglist_model.txt", 'w')
    for i in file_list:
        f.write(i + '\n')
    f.close()
...
#            cv2.rectangle(img, (int(boundingboxes[i][0]), int(boundingboxes[i][1])), (int(boundingboxes[i][2]), int(boundingboxes[i][3])), (0,255,0), 1)    
        ...
        img = image_trim(img, boundingboxes)
        cv2.imshow('img', img)
